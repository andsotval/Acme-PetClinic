...
class VisitControllerTests {
	...
	void setup() {
		...
		clinic.setAddress("Address " + TEST_CLINIC_ID);
		clinic.setCity("City " + TEST_CLINIC_ID);
		clinic.setName("Name " + TEST_CLINIC_ID);
		clinic.setTelephone("65432100" + TEST_CLINIC_ID);

		Visit pendingVisit = new Visit();
		pendingVisit.setId(TEST_PENDING_VISIT_ID);
		pendingVisit.setClinic(clinic);
		...
		acceptedVisit.setClinic(clinic);
		...
		BDDMockito.given(vetService.findVetsByClinicId(TEST_CLINIC_ID)).willReturn(Lists.newArrayList(vet));

		BDDMockito.given(visitService.findAllByPetId(TEST_PET_ID))
