...
class StayControllerTests {
	...
	void testShowStaysAccept() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("stays/listAllAccepted")).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.model().attributeExists("stays")).andExpect(MockMvcResultMatchers.view().name("stays/list"));
	}
	...
	void testAcceptStay() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("stays/accept/{stayId}", StayControllerTests.TEST_STAY_ID)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.view().name("redirect:/stays/listAllPending"));
	}
	...
	void testCancelStay() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("stays/cancel/{stayId}", StayControllerTests.TEST_STAY_ID)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.view().name("redirect:/stays/listAllPending"));
	}
	...
	void testStaySuccesfull() throws Exception {
		this.mockMvc
			.perform(MockMvcRequestBuilders.post("stays/save/{stayId}", StayControllerTests.TEST_STAY_ID).with(SecurityMockMvcRequestPostProcessors.csrf()).param("description", "Description").param("startDate", "2020-06-09").param("endDate", "2020-07-09"))
			.andExpect(MockMvcResultMatchers.model().attributeHasErrors("stay")).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.view().name("pets/createOrUpdateVisitForm"));
	}
