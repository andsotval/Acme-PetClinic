...
public class SuggestionUserControllerTest {
	...
	void testList() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/suggestion/user/list")).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.model().attributeExists("suggestions"))
			.andExpect(MockMvcResultMatchers.model().attribute("isTrash", false)).andExpect(MockMvcResultMatchers.view().name("suggestion/user/list"));
	}
	...
	void testDetailValueNotPresent() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/suggestion/user/details/{suggestionId}", TEST_SUGGESTION_ID_99)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.view().name("exception"));
	}
	...
	void testSavePositive() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post("/suggestion/user/save").with(SecurityMockMvcRequestPostProcessors.csrf()).param("name", "name1").param("description", "description1").param("created", "2020/05/26 12:00:00").param("isRead", "false")
			.param("isTrash", "false").param("isAvailable", "true")/* .param("user", "true") */).andExpect(MockMvcResultMatchers.status().is3xxRedirection()).andExpect(MockMvcResultMatchers.view().name("redirect:/suggestion/user/list"));
	}
	...
	void testDelete() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/suggestion/user/delete/{suggestionId}", TEST_SUGGESTION_ID_1)).andExpect(MockMvcResultMatchers.status().is3xxRedirection()).andExpect(MockMvcResultMatchers.view().name("redirect:/suggestion/user/list"));
	}

	@WithMockUser(value = "spring")
	@Test
	void testDeleteAll() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/suggestion/user/deleteAll")).andExpect(MockMvcResultMatchers.status().is3xxRedirection()).andExpect(MockMvcResultMatchers.view().name("redirect:/suggestion/user/list"));
	}
