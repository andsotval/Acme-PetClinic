...
@WebMvcTest(value = SuggestionUserController.class,
...
public class SuggestionUserControllerTest {
	...
	void setup() {
		...
		BDDMockito.given(suggestionService.findAllEntitiesAvailableByUsername("pepito"))
		...
		mockMvc
			.perform(
				MockMvcRequestBuilders.post("/suggestion/user/save").with(SecurityMockMvcRequestPostProcessors.csrf())
					.param("name", "name1").param("description", "description1").param("created", "2020/05/26 12:00:00")
					.param("isRead", "false").param("isTrash", "false").param("isAvailable", "true"))
			.andExpect(MockMvcResultMatchers.status().isOk())
			.andExpect(MockMvcResultMatchers.view().name("suggestion/user/list"));
	}
	...
	void testDeleteNegativeNotExistingSuggestion() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/suggestion/user/delete/{suggestionId}", TEST_SUGGESTION_ID_99))
			.andExpect(MockMvcResultMatchers.status().is3xxRedirection())
			.andExpect(MockMvcResultMatchers.view().name("redirect:/oups"));
	}
